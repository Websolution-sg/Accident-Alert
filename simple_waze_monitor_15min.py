...
def format_waze_accident_message(accident):
    """Format Waze accident using original detailed format - with enhanced location detection"""
    # Extract information
    street = accident.get('street', '')
    ...
    # Enhanced location formatting with intelligent fallbacks
    location_text = None
    
    # First priority: Use street name if available and meaningful
    ...
    # Second priority: Try to identify expressway from coordinates
    if not location_text and lat and lon:
        expressway = get_singapore_expressway_from_coords(lat, lon)
        if expressway:
            location_text = expressway
    
    # Third priority: Use area/estate mapping from coordinates  
    if not location_text and lat and lon:
        area = get_singapore_location_from_coords(lat, lon)
        if area:
            location_text = f"near {area}"
    
    # Fourth priority: Use city name if available
    if not location_text and city and city != 'Singapore':
        location_text = f"{city} area"
    
    # Final fallback: Generic location with coordinates if available
    if not location_text:
        if lat and lon:
            location_text = f"Singapore ({lat:.3f}, {lon:.3f})"
        else:
            location_text = "Singapore (location details unavailable)"
    
    # Build message using original detailed format
