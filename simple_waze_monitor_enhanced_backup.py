...
def format_waze_accident_message(accident):
    """Format Waze accident using @sgaccident descriptive style"""
    # Extract information
    street = accident.get('street', '')
    ...
    # Create @sgaccident style descriptive location
    accident_description = create_sgaccident_style_description(street, lat, lon)
    
    # Build message using @sgaccident format style
    message = f"{accident_description}\n"
...
def create_sgaccident_style_description(street, lat, lon):
    """Create @sgaccident style descriptive accident location"""
    
    # Try to identify expressway and create directional description
    if lat and lon:
        expressway_info = get_detailed_expressway_info(lat, lon)
        if expressway_info:
            return f"Accident on {expressway_info}"
    
    # Enhanced street-based description 
    if street and street.lower() != 'unknown location' and len(street.strip()) > 2:
        # Try to enhance street description with area information
        if lat and lon:
            area = get_singapore_location_from_coords(lat, lon)
            if area and area not in street:
                return f"Accident on {street} near {area}"
        return f"Accident on {street}"
    
    # Fallback to area-based description
    if lat and lon:
        area = get_singapore_location_from_coords(lat, lon)
        if area:
            return f"Accident near {area}"
        return f"Accident in Singapore ({lat:.4f}, {lon:.4f})"
    
    return "Accident in Singapore"

def get_detailed_expressway_info(lat, lon):
    """Get detailed expressway information with direction like @sgaccident"""
    # Check if on expressway with enhanced detection
    expressways = {
        # CTE (Central Expressway) with directional hints
        (1.29, 1.42, 103.82, 103.86): ("CTE", "Central Expressway", ["towards City", "towards Woodlands"]),
        # PIE (Pan Island Expressway) 
        (1.31, 1.35, 103.65, 103.95): ("PIE", "Pan Island Expressway", ["towards Jurong", "towards Changi"]),
        # AYE (Ayer Rajah Expressway)
        (1.26, 1.32, 103.72, 103.84): ("AYE", "Ayer Rajah Expressway", ["towards City", "towards Tuas"]),
        # BKE (Bukit Timah Expressway)
        (1.33, 1.44, 103.76, 103.82): ("BKE", "Bukit Timah Expressway", ["towards Woodlands", "towards City"]),
        # TPE (Tampines Expressway)
        (1.33, 1.39, 103.87, 103.96): ("TPE", "Tampines Expressway", ["towards SLE", "towards PIE"]),
        # SLE (Seletar Expressway)
        (1.38, 1.43, 103.82, 103.89): ("SLE", "Seletar Expressway", ["towards TPE", "towards BKE"]),
        # ECP (East Coast Parkway)
        (1.29, 1.32, 103.85, 104.00): ("ECP", "East Coast Parkway", ["towards City", "towards Changi"]),
        # KPE (Kallang-Paya Lebar Expressway)
        (1.32, 1.37, 103.86, 103.90): ("KPE", "Kallang-Paya Lebar Expressway", ["towards PIE", "towards ECP"])
    }
    
    for (lat_min, lat_max, lon_min, lon_max), (code, name, directions) in expressways.items():
        if lat_min <= lat <= lat_max and lon_min <= lon <= lon_max:
            # Try to determine direction based on coordinates
            direction = ""
            if len(directions) >= 2:
                # Simple heuristic: use coordinate position to suggest direction
                mid_lat = (lat_min + lat_max) / 2
                mid_lon = (lon_min + lon_max) / 2
                
                if "north" in directions[0].lower() or "south" in directions[0].lower():
                    direction = f" ({directions[0] if lat > mid_lat else directions[1]})"
                else:
                    direction = f" ({directions[0] if lon < mid_lon else directions[1]})"
            
            # Look for nearby landmarks to add "at" description
            landmark = get_nearby_landmark(lat, lon)
            if landmark:
                return f"{code}{direction} at {landmark}"
            else:
...
def get_nearby_landmark(lat, lon):
    """Get nearby landmark for more descriptive location (like @sgaccident style)"""
    # This is a simplified version - could be enhanced with more landmark data
    landmarks = {
        # Some key Singapore landmarks and exits
        (1.344, 103.860): "Braddell Road Entrance",
        (1.340, 103.855): "Bishan Exit", 
        (1.350, 103.865): "Ang Mo Kio Exit",
        (1.335, 103.850): "Toa Payoh Exit",
        (1.325, 103.845): "Novena Exit",
        (1.315, 103.840): "Newton Exit",
        (1.305, 103.835): "Orchard Exit",
        (1.295, 103.830): "River Valley Exit"
    }
    
    # Find closest landmark within reasonable distance (0.01 degrees ~ 1km)
    min_distance = float('inf')
    closest_landmark = None
    
    for (landmark_lat, landmark_lon), landmark_name in landmarks.items():
        distance = ((lat - landmark_lat) ** 2 + (lon - landmark_lon) ** 2) ** 0.5
        if distance < 0.01 and distance < min_distance:  # Within ~1km
