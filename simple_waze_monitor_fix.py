...
def format_waze_accident_message(accident):
    ...
    # Format location - EXPRESSWAY DETECTION FIRST, then original format
    # Check for expressway FIRST (regardless of street data quality)
    if lat and lon:
        expressway = get_singapore_expressway_from_coords(lat, lon)
        if expressway:
            expressway_name = expressway.split('(')[0].strip()  # Just get the name part
            # Add area information to expressway for better location context
            area = get_singapore_location_from_coords(lat, lon)
            if area and area not in expressway_name:
                location_text = f"{expressway_name}, {area}"
            else:
                location_text = expressway_name
        # If NOT on expressway, use original format
        elif street and street.lower() not in ['unknown location', 'unknown', ''] and len(street.strip()) > 2:
            # Use original simple format when we have good street data
            ...
            # Get area/neighborhood from coordinates as final fallback
            area = get_singapore_location_from_coords(lat, lon)
            if area:
                location_text = area
            else:
                location_text = f"coordinates {lat:.4f}, {lon:.4f}"
    # No coordinates available - use original text-based logic
    elif street and street.lower() not in ['unknown location', 'unknown', ''] and len(street.strip()) > 2:
        if city and city != 'Singapore':
            location_text = f"{street}, {city}"
        else:
