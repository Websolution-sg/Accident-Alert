...
def log_message(message):
    ...
    print(f"[{timestamp}] USER: {message}", flush=True)
...
async def setup_client():
    ...
    try:
        ...
        log_message(f"✅ Authenticated as: {me.username or me.first_name}")
        return client
    except Exception as e:
...
async def monitor_sgaccident_user(client):
    """Monitor @sgaccident channel and forward ALL messages (no filtering)"""
    log_message("🔍 Starting @sgaccident monitoring with user account (NO FILTERING)...")
    ...
    async def handler(event):
        try:
            ...
            if not text:
                log_message("⚠️  Skipping message with no text content")
                return
            
            message_id = f"user_sgaccident_{message.id}"
            
            if message_id in processed_ids:
                return
            
            log_message(f"📨 New @sgaccident message detected (no filtering)")
            log_message(f"📝 Content: {text[:100]}{'...' if len(text) > 100 else ''}")
            
            # Extract coordinates if available
            lat, lon = extract_coordinates_from_text(text)
            coordinates = (lat, lon) if lat and lon else None
            
            # Format message (no filtering - forward everything)
            formatted_message = format_user_accident_message(text, coordinates)
            
            # Send to target channel
            await client.send_message(TARGET_CHANNEL_ID, formatted_message, parse_mode='md')
            
            # Mark as processed
            processed_ids.add(message_id)
            save_user_processed_accidents(processed_ids)
            
            log_message(f"✅ Forwarded @sgaccident message (no filtering applied)")
            
        except Exception as e:
            log_message(f"❌ Error processing message: {e}")
    
    log_message("👂 Listening for @sgaccident messages...")
...
if __name__ == "__main__":
    print("🔧 USER-BASED TELEGRAM MONITOR SETUP")
    print("=" * 50)
    print("📋 Before running, you need to:")
    print("1. Go to https://my.telegram.org")
    print("2. Login with @pukiboi account")
    print("3. Create a new app to get API_ID and API_HASH")
    print("4. Edit this script and fill in:")
    print("   - API_ID = your_api_id")
    print("   - API_HASH = 'your_api_hash'")
    print("   - PHONE_NUMBER = '@pukiboi_phone_number'")
    ...
    if API_ID and API_HASH and PHONE_NUMBER:
        ...
        print("Edit the script and set API_ID, API_HASH, and PHONE_NUMBER")