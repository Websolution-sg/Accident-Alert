...
# Data storage files (VM paths)
PROCESSED_FILE = "/tmp/processed_accidents.json"
TELEGRAM_OFFSET_FILE = "/tmp/telegram_offset.json"

def log_message(message):
    """Log messages with timestamp, write to log file and immediate flush"""
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    log_entry = f"[{timestamp}] {message}"
    print(log_entry, flush=True)
    
    # Also write to log file for VM
    try:
        with open("/tmp/accident_monitor.log", 'a') as f:
            f.write(log_entry + '\n')
    except Exception:
...
def process_waze_accidents():
    ...
    try:
        ...
        for accident in accidents:
            ...
            # Geographic filtering disabled - accepting all locations
        ...
        for update in updates:
            ...
            if 'channel_post' in update:
                ...
                if chat_id == SGACCIDENT_CHAT_ID:
                    ...
                    # Content filtering disabled - accepting all messages with text
                    if text:
                        ...
                        # Malaysia filtering disabled - accepting all content
