...
def get_singapore_expressway_with_direction(lat, lon):
    """Get Singapore expressway with enhanced exit/junction context"""
    if not lat or not lon:
        return None
        
    # Enhanced expressway definitions with exit/junction context
    
    # PIE (Pan Island Expressway) - runs east-west, major exits
    if 1.31 <= lat <= 1.35 and 103.65 <= lon <= 103.95:
        if lon < 103.70:  # Western section
            direction = "towards Tuas"
            if lon < 103.68:
                exit_info = "(before Exit 1 Tuas)"
            else:
                exit_info = "(near Exit 2 Jurong East)"
        elif lon < 103.78:  # Western-central
            direction = "towards Tuas"
            exit_info = "(near Exit 3 Bukit Batok)"
        elif lon < 103.82:  # Central section
            direction = "towards Changi"
            exit_info = "(after Exit 4 Mount Pleasant)"
        elif lon < 103.88:  # Eastern-central
            direction = "towards Changi"
            exit_info = "(near Exit 5 Eunos)"
        else:  # Eastern section
            direction = "towards Changi"
            exit_info = "(before Exit 6 Simei)"
        expressway = f"PIE {direction} {exit_info}"
        
    # CTE (Central Expressway) - runs north-south, key junctions
    elif 1.29 <= lat <= 1.42 and 103.82 <= lon <= 103.86:
        if lat < 1.32:  # Southern section
            direction = "towards town"
            exit_info = "(near Exit 1 Kim Seng)"
        elif lat < 1.35:  # Central section
            direction = "towards town"
            exit_info = "(after Exit 3 Braddell)"
        elif lat < 1.38:  # North-central
            direction = "towards Woodlands"
            exit_info = "(near Exit 5 Ang Mo Kio)"
        else:  # Northern section
            direction = "towards Woodlands"
            exit_info = "(before Exit 7 Yishun)"
        expressway = f"CTE {direction} {exit_info}"
        
    # AYE (Ayer Rajah Expressway) - southern route, east-west
    elif 1.26 <= lat <= 1.32 and 103.72 <= lon <= 103.84:
        if lon < 103.75:  # Western section
            direction = "towards Jurong"
            exit_info = "(near Exit 1 Jurong)"
        elif lon < 103.78:  # Western-central
            direction = "towards Jurong"
            exit_info = "(after Exit 3 Yuan Ching)"
        elif lon < 103.81:  # Central section
            direction = "towards town"
            exit_info = "(near Exit 5 Clementi)"
        else:  # Eastern section
            direction = "towards town"
            exit_info = "(before Exit 7 Alexandra)"
        expressway = f"AYE {direction} {exit_info}"
        
    # BKE (Bukit Timah Expressway) - northwest, north-south
    elif 1.33 <= lat <= 1.44 and 103.76 <= lon <= 103.82:
        if lat < 1.36:  # Southern section
            direction = "towards town"
            exit_info = "(near Exit 1 Bukit Timah)"
        elif lat < 1.38:  # Central section
            direction = "towards town"
            exit_info = "(after Exit 2 Dairy Farm)"
        elif lat < 1.41:  # North-central
            direction = "towards Woodlands"
            exit_info = "(near Exit 4 Kranji)"
        else:  # Northern section
            direction = "towards Woodlands"
            exit_info = "(before Exit 5 Woodlands)"
        expressway = f"BKE {direction} {exit_info}"
    ...
    # TPE (Tampines Expressway) - northeast, east-west
        ...
        if lon < 103.89:  # Western section
            ...
            exit_info = "(near Exit 1 Whitley)"
        elif lon < 103.91:  # Central section
            direction = "towards SLE"
            exit_info = "(after Exit 2 Paya Lebar)"
        elif lon < 103.93:  # Eastern section
            direction = "towards Changi"
            exit_info = "(near Exit 4 Tampines)"
        else:  # Far eastern
            direction = "towards Changi"
            exit_info = "(before Exit 5 Loyang)"
        expressway = f"TPE {direction} {exit_info}"
    ...
    # SLE (Seletar Expressway) - north, east-west
        ...
        if lon < 103.84:  # Western section
            ...
            exit_info = "(near BKE junction)"
        elif lon < 103.86:  # Central section
            direction = "towards TPE"
            exit_info = "(after Exit 1 Yio Chu Kang)"
        else:  # Eastern section
            direction = "towards TPE"
            exit_info = "(before TPE junction)"
        expressway = f"SLE {direction} {exit_info}"
        
    # ECP (East Coast Parkway) - southeast coastal, east-west
    elif 1.29 <= lat <= 1.32 and 103.86 <= lon <= 103.96:
        if lon < 103.88:  # Western section
            direction = "towards town"
            exit_info = "(near Exit 1 Marina Bay)"
        elif lon < 103.91:  # Central section
            direction = "towards town"
            exit_info = "(after Exit 2 Tanjong Katong)"
        elif lon < 103.93:  # Eastern section
            direction = "towards Changi"
            exit_info = "(near Exit 4 Bedok)"
        else:  # Far eastern
            direction = "towards Changi"
            exit_info = "(before Exit 5 Changi)"
        expressway = f"ECP {direction} {exit_info}"
    ...
    # KPE (Kallang-Paya Lebar Expressway) - generally north-south
        ...
        if lat < 1.32:  # Southern section
            ...
            exit_info = "(near Exit 1 Marina Bay)"
        elif lat < 1.33:  # South-central
            direction = "towards Marina Bay"
            exit_info = "(after Exit 2 Kallang)"
        elif lat < 1.34:  # Central section
            direction = "towards Defu"
            exit_info = "(near Exit 4 Macpherson)"
        else:  # Northern section
            direction = "towards Defu"
            exit_info = "(before Exit 6 Defu)"
        expressway = f"KPE {direction} {exit_info}"
